  ...
  const contentJSX = React.useMemo(() => {
    if (isLoggedIn) {
      ...
      if (location.pathname.includes('login'))
        return (
          <div className='ml-auto mr-10'>
            <Button onClick={() => {navigate('/register/')}} color="success" variant="contained">Đăng ký</Button>
          </div>
        )
      else if (location.pathname.includes('register'))
          return (
            <div className='ml-auto mr-10'>
              <Button onClick={() => {navigate('/login/')}} color="success" variant="contained">Đăng nhập</Button>
            </div>
          )
    }
